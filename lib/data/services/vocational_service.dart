...
class VocationalService implements VocationalRepository {
  ...
  @override
  Future<PaginatedResponse<Map<String, dynamic>>> getPklGrades(
      {int page = 1}) async {
    final r = await _dioClient
        .get(ApiEndpoints.pklGrades, queryParameters: {'page': page});
    return PaginatedResponse.fromJson(r.data, (j) => j as Map<String, dynamic>);
  }

  @override
  Future<void> submitPklGrade(Map<String, dynamic> data) async {
    await _dioClient.post(ApiEndpoints.pklGrades, data: data);
  }
}
